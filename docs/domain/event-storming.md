...
- ✅ [D-5] `access-control` 検証カテゴリ（未認証アクセス拒否の負テスト）を**実装・検証済**（[[sync]] #5。
  `verify/accessControl.ts`・匿名プローブで guard 判定。open-pms で誤検知0・真陽性検出を確認）。未コミット。
  残スコープ外：ロール間の水平権限（IDOR）は別 divergence 候補。
